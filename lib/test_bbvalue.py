...
class TestSimpleExpansions(unittest.TestCase):
    ...
    def test_one_var(self):
        ...
        self.assertEqual(traverse.resolve(val, self.d), "value of foo")

    def test_indirect_one_var(self):
        val = bbvalue.bbparse("${${foo}}")
        self.assertEqual(traverse.resolve(val, self.d), "value of 'value of foo'")

    def test_indirect_and_another(self):
        val = bbvalue.bbparse("${${foo}} ${bar}")
        self.assertEqual(traverse.resolve(val, self.d), "value of 'value of foo' value of bar")

    def test_python_snippet(self):
        val = bbvalue.bbparse("${@5*12}")
        self.assertEqual(traverse.resolve(val, self.d), "60")

    def test_expand_in_python_snippet(self):
        val = bbvalue.bbparse("${@'boo ' + '${foo}'}")
        self.assertEqual(traverse.resolve(val, self.d), "boo value of foo")

    def test_python_snippet_getvar(self):
        val = bbvalue.bbparse("${@d.getVar('foo', True) + ' ${bar}'}")
        self.assertEqual(traverse.resolve(val, self.d), "value of foo value of bar")

    def test_python_snippet_syntax_error(self):
        self.d.setVar("FOO", "${@foo = 5}")
        val = bbvalue.bbvalue("FOO", self.d)
        self.assertRaises(SyntaxError, traverse.resolve, val, self.d)

    def test_python_snippet_runtime_error(self):
        self.d.setVar("FOO", "${@int('test')}")
        val = bbvalue.bbvalue("FOO", self.d)
        self.assertRaises(traverse.PythonExpansionError, traverse.resolve, val, self.d)
    ...
    def test_reference_undefined_var(self):
        ...
        self.assertEqual(traverse.resolve(val, self.d), "${undefinedvar} meh")
...
class TestNestedExpansions(unittest.TestCase):
    ...
    def test_refs(self):
        ...
        self.assertEqual(traverse.resolve(val, self.d), "187")

    def test_python_refs(self):
        val = bbvalue.bbparse("${@${@3}**2 + ${@4}**2}")
        self.assertEqual(traverse.resolve(val, self.d), "25")

    def test_ref_in_python_ref(self):
        val = bbvalue.bbparse("${@'${foo}' + 'bar'}")
        self.assertEqual(traverse.resolve(val, self.d), "foobar")

    def test_python_ref_in_ref(self):
        val = bbvalue.bbparse("${${@'f'+'o'+'o'}}")
        self.assertEqual(traverse.resolve(val, self.d), "foo")
    ...
    def test_deep_python_nesting(self):
        ...
        self.assertEqual(traverse.resolve(val, self.d), str(depth + 1))
